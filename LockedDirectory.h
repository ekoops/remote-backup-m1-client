...
public:
    ...
    virtual bool insert(boost::filesystem::path const &path) override;
    ...
    virtual bool contains(boost::filesystem::path const& path) override;
    virtual std::pair<bool, bool> contains_and_match(boost::filesystem::path const& path, Metadata const& metadata) override;
    ...
    virtual void for_each_if(std::function<bool(boost::filesystem::path const &)> const &pred,
                                std::function<void(std::pair<boost::filesystem::path, Metadata> const&)> const &action) override;
};
