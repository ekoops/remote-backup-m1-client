...
void operations_handler(PendingOperationsQueue &poq,
              std::shared_ptr<WatchedDirectory> const &wd_ptr) {
    while (true) {
...
int main(int argc, char const * const argv[]) {
    if (argc < 2) {
        std::cerr << "Usage: ./name path_to_watch";
        return -1;
    }
    ...
    if (!exists(path_to_watch) ||
        ...
        std::cerr << "Path provided doesn't exist is not a directory or permission denied";
        return -2;
    }

    std::shared_ptr<WatchedDirectory> wd_ptr = WatchedDirectory::get_instance(path_to_watch);
    FileWatcher fw{wd_ptr, std::chrono::milliseconds(3000)};
    PendingOperationsQueue poq{};
    std::thread t(operations_handler, std::ref(poq), std::cref(wd_ptr));
